import {
  ...
  export default function ReturnedFolder({route}) {
    ...
    >
        <div style={{color:'darkblue',fontWeight:'bold',fontSize:24,marginTop:20}}>Evaluation</div>
      ...
      >
        ...
        <tr
          ...
          >
           ...
           <td style={{fontWeight:'bold',fontSize:18}}>{item.title}</td>
           </tr>
...
return(
  ...
  {state.data.Evaluation?.map((item,index)=>{
            if(item.title.includes("Mid") || item.title.includes("Sessional")){
return(
      <table
        cellSpacing={9}
        cellPadding={6}
